...
-- versione 4 --
RISOLTO il problema della lettura della /home
Possibilita' di inserire un path con terminazione '/'
*/
...
int main(int argc, char *argv[1]){
	...
	char path_time[100];
	strcpy(path_time, argv[1]);
	if(path_time[strlen(path_time)-1] == '/'){
		strncpy(path, path_time, strlen(path_time)-1);
	}else{
		strcpy(path, argv[1]);
	}
